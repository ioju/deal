var slide1Height=$("#step-slide1 .collaborate-form-step").height();var slide2Height=$("#step-slide2 .collaborate-form-step").height();console.log(slide1Height);console.log(slide2Height);const prevButtonHTML='<div class="previous w-slider-arrow-left" role="button" tabindex="0" aria-controls="w-slider-mask-0" aria-label="previous slide" style="display: none;"><div id="prevButton" class="previous-button">← Back</div></div>';const nextButtonHTML='<div class="next roviniete w-slider-arrow-right" role="button" tabindex="0" aria-controls="w-slider-mask-0" aria-label="next slide" style="top: -468.516px;"><div id="nextButton" class="next-button button">Continua</div></div>';var removedCategoryRowsNoTimes=0;const base_url="https://app-dev.autodeal.ro/api/v1";const base_url_mobilpay="https://app-dev.autodeal.ro";const token="100644|9ZcG86c7wzuhPBrnIJ9D1DwsrZOSsgvL7DJbdp9l";const errorColor="hsl(350 100% 13.5%)";const errorBg="hsl(350 100% 66.5%)";var categories=[];var availabilities=[];const images={"A-Autoturism":"https://uploads-ssl.webflow.com/60b35cb1a44c2d844c8e31f9/6332e5866e57b7cee8f870d5_sedan%201.png","B-Mf 0,0t<MTMA<=3,5t":"https://uploads-ssl.webflow.com/60b35cb1a44c2d844c8e31f9/6332e586ea937c33d185a284_delivery-truck%201.png"};function removeRadiochildrenOnce(parentId){if(removedCategoryRowsNoTimes>=1){return}const parent=document.getElementById(parentId);for(const row of[...parent.children]){if(row.innerHTML.includes("radio-button-label-2")){parent.removeChild(row)}}if(parentId==="category-table"){removedCategoryRowsNoTimes+=1}}function getVignetteCategories(){var request=new XMLHttpRequest;request.open("GET",`${base_url}/vignette-categories-web`,false);request.setRequestHeader("Authorization",`Bearer ${token}`);request.onload=function(){var data=JSON.parse(this.response);if(request.status>=200&&request.status<400){categories=data.data}else{}};request.send()}function getVignetteCategoryPrices(categoryId){console.log("aici");var request=new XMLHttpRequest;request.open("GET",`${base_url}/vignette-categories/${categoryId}/prices`,false);request.setRequestHeader("Authorization",`Bearer ${token}`);request.setRequestHeader("Permissions-Policy","ch-ua-form-factor");request.onload=function(){var data=JSON.parse(this.response);if(request.status>=200&&request.status<400){availabilities=data.data;addPrices()}else{}};request.send()}function addNextPrevButtonsAsChildOfId(elemId){const parent=document.getElementById(elemId);if(!parent.innerHTML.includes(prevButtonHTML)){parent.innerHTML+=prevButtonHTML}if(!parent.innerHTML.includes(nextButtonHTML)){parent.innerHTML+=nextButtonHTML}}function removeNextPrevButtonsAsChildOfId(elemId){const parent=document.getElementById(elemId);if(parent.innerHTML.includes(prevButtonHTML)){parent.innerHTML=parent.innerHTML.replace(prevButtonHTML,"")}if(parent.innerHTML.includes(nextButtonHTML)){parent.innerHTML=parent.innerHTML.replace(nextButtonHTML,"")}}function addCategoriesRow(){const parent=document.getElementById("category-table");var row=document.createElement("div");addClass(row,"rov-row");addClass(row,"w-clearfix");for(let i=0;i<categories.length;i++){if(i%4==0&&i!=0){parent.appendChild(row);row=document.createElement("div");addClass(row,"rov-row");addClass(row,"w-clearfix")}var category=categories[i];var categoryDivInput=document.createElement("div");addClass(categoryDivInput,"input-group input-fullwidth rovinieta-input");var categoryDivWrap=document.createElement("div");addClass(categoryDivWrap,"extra-option-wrap");var label=document.createElement("label");addClass(label,"radio-button-field-2 w-clearfix w-radio");var img=document.createElement("img");addClass(img,"image-17");img.setAttribute("src",images[category.code]);img.setAttribute("loading","lazy");img.setAttribute("width","64");img.setAttribute("alt",category.name);var categoryDivRadioInput=document.createElement("div");addClass(categoryDivRadioInput,"w-form-formradioinput w-form-formradioinput--inputType-custom radio-button-2 align-right w-radio-input");if(category.code=="A-Autoturism"){addClass(categoryDivRadioInput,"w--redirected-checked");localStorage.setItem("selectedCategory",category.id)}var input=document.createElement("input");input.setAttribute("type","radio");input.setAttribute("name","auto");input.setAttribute("data-name","auto");input.setAttribute("style","opacity:0;position:absolute;z-index:-1");input.setAttribute("value",category.id);input.setAttribute("id","auto"+i);var span=document.createElement("span");addClass(span,"radio-button-label-2 radio-image w-form-label");span.setAttribute("for","auto"+i);var strong=document.createElement("strong");addClass(strong,"bold-text-3");strong.innerHTML=category.name;span.appendChild(strong);label.appendChild(img);label.appendChild(categoryDivRadioInput);label.appendChild(input);label.appendChild(span);label.addEventListener("click",function(event){if(event.target.type=="radio"){localStorage.setItem("selectedCategory",event.target.value)}});categoryDivWrap.appendChild(label);categoryDivInput.appendChild(categoryDivWrap);row.appendChild(categoryDivInput)}parent.appendChild(row)}function removeSuccess(seconds=0){setTimeout(function(){container=document.getElementById("successMessageSubmit");if(container==null)return;container.style.display="none";form=document.getElementById("wf-form-rovinieta-detalii");form.style.display="block"},seconds*1e3)}function addPrices(){const parent=document.getElementById("rovList");parent.innerHTML="";var row=document.createElement("div");addClass(row,"w-clearfix");for(let i=0;i<availabilities.length;i++){var availability=availabilities[i];var categoryDivInput=document.createElement("div");addClass(categoryDivInput,"input-group input-fullwidth rovinieta-input");var categoryDivWrap=document.createElement("div");addClass(categoryDivWrap,"extra-option-wrap");var label=document.createElement("label");addClass(label,"radio-button-field-2 fullwidth w-clearfix w-radio");var categoryDivRadioInput=document.createElement("div");addClass(categoryDivRadioInput,"w-form-formradioinput w-form-formradioinput--inputType-custom radio-button-2 w-radio-input");if(i==0){addClass(categoryDivRadioInput,"w--redirected-checked");localStorage.setItem("selectedAvailability",availability.availability_code)}var input=document.createElement("input");input.setAttribute("type","radio");input.setAttribute("name","availability");input.setAttribute("data-name","availability");input.setAttribute("style","opacity:0;position:absolute;z-index:-1");input.setAttribute("value",availability.availability_code);input.setAttribute("id","availability"+i);var span=document.createElement("span");addClass(span,"radio-button-label-2 radio-button-label-3 w-form-label");span.setAttribute("for","availability"+i);var strong=document.createElement("strong");addClass(strong,"bold-text-3");strong.innerHTML=availability.availability_name;span.appendChild(strong);var div13=document.createElement("div");addClass(div13,"div-block-13");var text13=document.createElement("div");addClass(text13,"text-block-13");text13.innerHTML=availability.value+" "+availability.currency_code;var eur=document.createElement("div");addClass(eur,"eur");eur.innerHTML=availability.authority_value+" "+availability.authority_currency_code;div13.appendChild(text13);div13.appendChild(eur);label.appendChild(categoryDivRadioInput);label.appendChild(input);label.appendChild(span);label.appendChild(div13);label.addEventListener("click",function(event){if(event.target.type=="radio"){localStorage.setItem("selectedAvailability",event.target.value)}});categoryDivWrap.appendChild(label);categoryDivInput.appendChild(categoryDivWrap);row.appendChild(categoryDivInput)}parent.appendChild(row)}let toastContainer;function generateToast({message,background="#00214d",color="#fffffe",length="3000ms"}){toastContainer.insertAdjacentHTML("beforeend",`<p class="toast" 
